...
const extractLine = (
  ...
  let lastWasBox = false; // true when last node was a box (span continuation)
  ...
  // line height and baseline from the first font's OS/2 metrics
  const firstFont = fonts[0];
  const m = firstFont && getMetrics ? getMetrics(firstFont) : null;
  const lineHeight = m ? m.ascender - m.descender + m.lineGap : 0;
  const baseline = m ? m.ascender : 0;
