...
import { ComposedParagraph, Font } from './types';
  ...
  for (const line of composed) {
    ...
    if (line.direction === 'rtl') {
      ...
      const wordWidths = line.wordRuns.map((segs) =>
        segs.reduce((sum, seg) => sum + measurer.measure(seg.text, seg.font), 0),
      );

      let rightEdge = origin.x + line.lineWidth;
...
const getUnitsPerEm = (fontEngine: FontEngine, fontId: string, fontSize: number): number => {
  if (!metricsCache.has(fontId)) {
    metricsCache.set(fontId, fontEngine.getFontMetrics(fontId, fontSize).unitsPerEm);
  }
