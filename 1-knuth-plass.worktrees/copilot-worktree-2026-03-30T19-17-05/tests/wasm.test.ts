...
import { FORCED_BREAK, PROHIBITED } from '../src/types';
...
const box = (w: number, t: string) => ({ type: 'box', width: w, content: t, font: FONT });
const glue = (w: number, st: number, sh: number) => ({ type: 'glue', kind: 'word', width: w, stretch: st, shrink: sh });
const termination = () => ({ type: 'glue', kind: 'termination', width: 0, stretch: 1e6, shrink: 0 });
const forced = () => ({ type: 'penalty', width: 0, penalty: FORCED_BREAK, flagged: false });
...
const MOCK_PARA_2LINE = {
  nodes: [
    box(30, 'The'), glue(6, 8, 2),
    box(25, 'quick'), glue(6, 8, 2),
    box(35, 'brown'), glue(6, 8, 2),
    box(20, 'fox'),
    termination(), forced(),
  ],
  lineWidth: 80,
...
const MOCK_PARA_1LINE = {
  nodes: [
    box(30, 'hello'), glue(6, 3, 2), box(25, 'world'),
    termination(), forced(),
  ],
  lineWidth: 200,
  tolerance: 3,
...
describe('Phase 2 — compute_breakpoints_wasm result shape', () => {
  ...
  it('returns ok object with active list', () => {
    const result = JSON.parse(wasm.compute_breakpoints_wasm(toWasmJson(MOCK_PARA_2LINE)));
  ...
  it('returns error object when paragraph cannot be set', () => {
    ...
    const result = JSON.parse(wasm.compute_breakpoints_wasm(toWasmJson(impossible)));
...
describe('Phase 2 — equivalence with TypeScript computeBreakpoints', () => {
  ...
  it('optimal break position matches TypeScript', () => {
    ...
    const rs = JSON.parse(wasm.compute_breakpoints_wasm(toWasmJson(MOCK_PARA_2LINE)));
  ...
  it('totalDemerits matches TypeScript within 1e-6', () => {
    const ts = computeBreakpoints(MOCK_PARA_2LINE as any);
    const rs = JSON.parse(wasm.compute_breakpoints_wasm(toWasmJson(MOCK_PARA_2LINE)));
    const diff = Math.abs(rs.ok.active[rs.ok.optimalIndex].totalDemerits - ts.node.totalDemerits);
  ...
  it('ratio matches TypeScript within 1e-6', () => {
    const ts = computeBreakpoints(MOCK_PARA_2LINE as any);
    const rs = JSON.parse(wasm.compute_breakpoints_wasm(toWasmJson(MOCK_PARA_2LINE)));
    const diff = Math.abs(rs.ok.active[rs.ok.optimalIndex].ratio - ts.node.ratio);
    expect(diff).toBeLessThan(1e-6);
  });
  ...
  it('per-line lineWidths: optimal position matches TypeScript', () => {
    ...
    const rs = JSON.parse(wasm.compute_breakpoints_wasm(toWasmJson(MOCK_PARA_LINEWIDTHS)));
    expect(rs.ok.active[rs.ok.optimalIndex].position).toBe(ts.node.position);
    expect(rs.ok.active[rs.ok.optimalIndex].line).toBe(ts.node.line);
  });
...
describe('Phase 3 — equivalence with TypeScript traceback', () => {
  ...
  it('break positions match TypeScript traceback', () => {
    ...
    const rs = JSON.parse(wasm.traceback_wasm(toWasmJson(MOCK_PARA_2LINE))).ok.breaks;
  ...
  it('1-line paragraph: single break matches TypeScript', () => {
    ...
    const rs = JSON.parse(wasm.traceback_wasm(toWasmJson(MOCK_PARA_1LINE))).ok.breaks;
    expect(rs).toHaveLength(ts.length);
  ...
  it('per-line lineWidths: positions and ratios match TypeScript', () => {
    ...
    const rs = JSON.parse(wasm.traceback_wasm(toWasmJson(MOCK_PARA_LINEWIDTHS))).ok.breaks;
    expect(rs).toHaveLength(ts.length);
    for (let i = 0; i < ts.length; i++) {
      expect(rs[i].position).toBe(ts[i].position);
      expect(Math.abs(rs[i].ratio - ts[i].ratio)).toBeLessThan(1e-6);
...
const P4_FONT_12 = {
  id: P4_FONT_ID, size: 12, weight: 400, style: 'normal', stretch: 'normal',
};
...
describe('Phase 4 — font shaping via rustybuzz', () => {
  ...
  it('measure_text_wasm succeeds after register_font', () => {
    const rs = JSON.parse(wasm.measure_text_wasm('hello', JSON.stringify(P4_FONT_12)));
  ...
  it('empty string returns width = 0', () => {
    const rs = JSON.parse(wasm.measure_text_wasm('', JSON.stringify(P4_FONT_12)));
  ...
  it('letterSpacing adds between glyphs (not after last)', () => {
    ...
    const text = 'ab';
    const rsLS = JSON.parse(wasm.measure_text_wasm(text, JSON.stringify(fontWithLS))).ok.width;
    const rsNo = JSON.parse(wasm.measure_text_wasm(text, JSON.stringify(fontNoLS))).ok.width;
  ...
  it('space width matches TypeScript realSpace within 1e-10', () => {
    ...
    const rs = JSON.parse(wasm.space_metrics_wasm(JSON.stringify(P4_FONT_12))).ok;
  ...
  it('font_metrics_wasm returns all 7 required fields', () => {
    const rs = JSON.parse(wasm.font_metrics_wasm(JSON.stringify(P4_FONT_12))).ok;
    for (const key of ['unitsPerEm', 'ascender', 'descender', 'xHeight', 'capHeight', 'lineGap', 'baselineShift']) {
  ...
  it('superscript baselineShift > 0, subscript < 0, normal = 0', () => {
    ...
    const rsNorm = JSON.parse(wasm.font_metrics_wasm(JSON.stringify(P4_FONT_12))).ok;
