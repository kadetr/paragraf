...
- `compile()` — 11-step single-document compilation pipeline:
  ...
  9. `composeDocument()` with overflow line counting
  10. `layoutDocument()` placing composed lines into pages
  11. Render to PDF (`renderDocumentToPdf`), SVG (`renderToSvg` per item), or `RenderedDocument`
...
- 63 tests (27 unit, 36 integration)
