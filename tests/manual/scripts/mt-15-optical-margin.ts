...
export async function run(): Promise<{ name: string; svg: string }[]> {
  ...
  const svg = renderToSvg(renderedOn, fontEngine, {
    width: PAGE_W,
    height: PAGE_H,
  });
  return [{ name: 'mt-15-optical-margin', svg }];
}

// ─── CLI entry point ──────────────────────────────────────────────────────────

if (process.argv[1]?.endsWith('mt-15-optical-margin.ts')) {
  ...
  console.log(
    `  OMA-on:  ${outOn.lines.length} lines, xVar=${varOn.toFixed(4)}`,
  );
