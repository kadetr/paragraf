#!/usr/bin/env tsx
// manual/scripts/mt-02-rtl-hebrew.ts
// MT-02 — RTL Hebrew Paragraph.
// Checks: right-to-left flow, word order, no hyphenation.
//
// Run:  tsx tests/manual/scripts/mt-02-rtl-hebrew.ts
...
for (const line of out.lines) {
  if (line.direction !== 'rtl') {
    console.log(
      `  FAIL  line ${out.lines.indexOf(line)}: direction='${line.direction}', expected 'rtl'`,
    );
    failures++;
  }
}

// Check no hyphenation
for (const line of out.lines) {
  if (line.hyphenated) {
    console.log(
      `  FAIL  line ${out.lines.indexOf(line)}: hyphenated=true in RTL paragraph`,
    );
    failures++;
  }
}
...
// Check baseline consistency: no unexpected vertical offsets in plain Hebrew text
{
  let ok = true;
  for (const [li, rl] of rendered.entries()) {
    for (const seg of rl.segments) {
...
console.log(`  No hyphenation: ${out.lines.every((l) => !l.hyphenated)}`);
...
writeSvg('mt-02-rtl-hebrew.svg', svg);
const pdf = await renderToPdf(rendered, fontEngine, {
  width: PAGE_W,
  height: PAGE_H,
  preDraw: (doc) => drawTestHeader(doc, 'MT-02'),
});
writePdf('mt-02-rtl-hebrew.pdf', pdf);
...
const metrics: TestMetrics = {
  ...
  extra: {
    direction: 'rtl',
    allRtl: out.lines.every((l) => l.direction === 'rtl'),
  },
};

writeJson('mt-02-rtl-hebrew.metrics.json', metrics);
